...
## experimenting with calculating EF 0q by hand 
# jost 2010
jost_even <- data %>% 
  left_join(rich_est %>%  separate(., Assemblage, c("EasementID", "Year"), sep = "_")) %>% 
  group_by(EasementID) %>%  
  mutate(site_abund = sum(Total)) %>% 
  mutate(site_rich = n_unique(Family)) %>% 
  group_by(EasementID, Family, Sample) %>% 
  mutate(spp_abund = sum(Total)) %>% 
  mutate(freq = spp_abund/site_abund) %>% 
  mutate(indiv_inequal = (freq/1/site_rich)) %>% 
  mutate(fam_inequal = indiv_inequal^spp_abund) %>% 
  group_by(EasementID) %>% 
  mutate(inequal_site = prod(fam_inequal)) %>% 
  mutate(geo_mean = inequal_site^(1/site_abund)) 
