...
# This csv was downloaded from the Google Doc data entry spreadsheet on August 2021
...
abundance_family %>% 
  ggplot(aes(Year, abundance, fill = "Family")) +
  geom_boxplot() +
  theme_classic()+
  labs(title = "Insect Abundance \n per Year")+
  xlab("\n Year") +
  ylab("Total Abundance")+
  geom_jitter(alpha=.4, width = .1, size = 3)

fit2 <- lm(abundance ~ Year + RestorationCategory, data = abundance_family)
summary(fit2)
